...
export default function Sidebar({ isCollapsed, onToggle, scrolled }) {
    ...
    return (
        <aside className={`fixed left-0 bottom-0 z-30 hidden flex-col justify-between border-r border-rule bg-paper/50 backdrop-blur-md py-6 md:flex animate-in fade-in slide-in-from-left-8 transition-all duration-300 ease-out ${scrolled ? 'top-[56px]' : 'top-[92px]'} ${isCollapsed ? 'w-[70px] px-2' : 'w-60 px-4'}`}>
